...
.toolbar{position:sticky;top:0;z-index:5;display:grid;grid-template-columns:1fr auto 1fr;align-items:center;padding:12px 20px;color:#fff;background:var(--navy);box-shadow:0 4px 18px #0003}.toolbar strong{color:var(--gold)}
.toolbar button{justify-self:end;border:0;border-radius:6px;padding:10px 18px;color:var(--navy);background:var(--gold);font-weight:800;cursor:pointer}
.report-back{justify-self:center;padding:8px 15px;border:1px solid #e7b93f80;border-radius:999px;color:#fff;background:#ffffff0a;font-size:12px;font-weight:800;text-decoration:none}.report-back:hover{color:var(--gold);border-color:var(--gold);background:#ffffff12}
...
@media(max-width:900px){.page{width:calc(100% - 20px);margin:10px;padding:20px;overflow:visible}.cover-grid{grid-template-columns:1fr}.groups{grid-template-columns:1fr}.player-header{align-items:flex-start;gap:12px;flex-direction:column}.summary{flex-wrap:wrap}footer{position:static;margin-top:20px}.toolbar{display:flex;justify-content:space-between;gap:10px}.toolbar button{padding:8px}.report-back{position:fixed;left:12px;bottom:calc(12px + env(safe-area-inset-bottom,0px));z-index:10;padding:9px 13px;background:#101a31f2;box-shadow:0 6px 20px #0004;backdrop-filter:blur(8px)}}
...
<div class="toolbar"><div><strong>Historia punktów</strong> · faza grupowa · 24 graczy</div><a class="report-back" href="hso.html" aria-label="Wróć do strony głównej">← WRÓĆ</a><button onclick="window.print()">DRUKUJ / ZAPISZ PDF</button></div>
